...
class Users2 {
	...
	public static function SetDogeAddress($userID, $address) {
		DB::Execute("UPDATE users2 SET dogeaddress = '" . $address . "' WHERE id = " . DB::DQ($userID) . ";");
	}

	public static function SetWavesAddress($userID, $address) {
		DB::Execute("UPDATE users2 SET wavesaddress = '" . $address . "' WHERE id = " . DB::DQ($userID) . ";");
	}

    public static function SetWavesPrivatekey($userID, $privatekey) {
		DB::Execute("UPDATE users2 SET privatekey = '" . $privatekey . "' WHERE id = " . DB::DQ($userID) . ";");
	}

	public static function GetWavesAddress($userID) {
		return DB::SingleRow("SELECT wavesaddress FROM users2 WHERE id = " . DB::DQ($userID) . ";");
	}

	public static function GetWavesPrivatekey($userID) {
		return DB::SingleRow("SELECT privatekey FROM users2 WHERE id = " . DB::DQ($userID) . ";");
	}
}
